...
window.addEventListener('load', (e) => {
  ...
  kereso.addEventListener('click', (event) => { 
    kereso.value = ''
    refreshVersenyzok()
  })
...
function refreshVersenyzok() {
  ...
  versenyzok.filter(
    ...
    ve.firstChild.querySelector('.kozeledik-button').addEventListener('click', (event) => {
...
function refreshKozeledok() {
  ...
  for (const v of kozeledik) {
    ...
    ve.firstChild.querySelector('.nev').textContent = v.nev;
    ve.firstChild.querySelector('.befuto-button').addEventListener('click', (event) => {
...
function refreshBefutottak() {
  ...
  for (const v of befutott) {
    ...
    ve.firstChild.querySelector('.nev').textContent = v.nev;
    ve.firstChild.querySelector('.ido').textContent = new Intl.DateTimeFormat('hu-HU', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: 'numeric',
...
function backupResults(befutott) {
  window.localStorage.setItem('endResult', JSON.stringify(befutott));
}
